...
if ($DataBasetoMonitor) {
	...
	foreach ($susDB in $sDataBasetoMonitor) 
			{                             
                ...
                if ($skipDb | ? {$_ -like $db}) 
                    {
                    write-host "DB is on Skip List, Move Will not be resumed for DB"$db
                    } 
               else {
                    ...
                    Write-Host "**************************************************"
                    write-host "DB size still high:$DB"
				    write-host "Moves will be Auto Resume when Drive is 95% Free."
